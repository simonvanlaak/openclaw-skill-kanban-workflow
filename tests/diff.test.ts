...
describe('diffWorkItems', () => {
  it('emits deleted, created, stage changed, then updated in sorted id order', () => {
    const prev: Record<string, WorkItem> = {
      a: { id: 'a', title: 'A', stage: Stage.fromAny('queued'), labels: [], raw: {} },
      b: { id: 'b', title: 'B', stage: Stage.fromAny('queued'), labels: [], raw: {} },
      d: { id: 'd', title: 'D', stage: Stage.fromAny('queued'), labels: ['x'], raw: {} },
    };

    const curr: Record<string, WorkItem> = {
      b: { id: 'b', title: 'B', stage: Stage.fromAny('in-progress'), labels: [], raw: {} },
      c: { id: 'c', title: 'C', stage: Stage.fromAny('queued'), labels: [], raw: {} },
      d: { id: 'd', title: 'D2', stage: Stage.fromAny('queued'), labels: ['x'], raw: {} },
    };
